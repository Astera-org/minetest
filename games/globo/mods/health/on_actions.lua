...
local function quick_physics(player, health, energy, thirst, hunger, temperature)
    local mov = 0
    local jum = 0
    mov, jum, _, _, _= calcHealthMods(player, health, energy, thirst, hunger, temperature)
...
minetest.register_globalstep(function(dtime)
    ...
    if timer > interval then
      ...
      for _,player in ipairs(minetest.get_connected_players()) do
        ...
        --cap energy etc
        energy=bound(energy,0,1000)
        thirst=bound(thirst,0,1000)
        hunger=bound(hunger,0,1000)
